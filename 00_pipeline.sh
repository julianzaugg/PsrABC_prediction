...
#
#   # Add custom tree references, e.g. selected Wells et al. references
#   bash 00_pipeline.sh [same args] \
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --custom-tree-refs)         CUSTOM_TREE_REFS="$2";          shift 2 ;;
...
[[ -n "$CUSTOM_TREE_REFS" && ! -f "$CUSTOM_TREE_REFS" ]] && err "Custom tree reference FASTA not found: $CUSTOM_TREE_REFS"
...
log "Custom refs  : ${CUSTOM_TREE_REFS:-not supplied}"
...
# =============================================================================
# STEP 6: Download reference sequences and prepare optional custom references
# =============================================================================
REFS_FAA="$OUTDIR/06_references/references_all.faa"
BASE_REF_METADATA="$OUTDIR/06_references/reference_metadata.tsv"
TREE_REFS_FAA="$OUTDIR/06_references/tree_references_all.faa"
TREE_REF_METADATA="$OUTDIR/06_references/reference_metadata_with_custom.tsv"
...
# Build the exact tree-reference FASTA and metadata table used downstream.
# Any user-supplied --custom-tree-refs FASTA is appended to the standard
# references, and minimal metadata is generated from FASTA labels so
# 06_build_summary.py can recognise them as tree refs.
cp "$REFS_FAA" "$TREE_REFS_FAA"
cp "$BASE_REF_METADATA" "$TREE_REF_METADATA"

if [[ -n "$CUSTOM_TREE_REFS" ]]; then
    log "  Adding custom tree references: $CUSTOM_TREE_REFS"
    cat "$CUSTOM_TREE_REFS" >> "$TREE_REFS_FAA"

    # Auto-generate reference metadata from custom FASTA labels.
    # Expected/simple naming scheme, matching built-in references:
    #   >Family_Descriptive_name__accession_or_original_id
    # Examples:
    #   >PsrA_My_reference__ABC123
    #   >PsrAPhsASrrA_Wells_01__GCA_000724175.1_42_1_3082_4
    #   >bSreASoeA_Wells_01__GCA_...
    # The first underscore-delimited field is treated as the protein/family/clade
    # anchor used by 06_build_summary.py for tree-clade assessment.
    grep '^>' "$CUSTOM_TREE_REFS" | sed 's/^>//; s/[[:space:]].*$//' | \
        awk 'BEGIN{OFS="\t"}
             {
               label=$1;
               accession=$1;
               sub(/__.*/, "", label);
               if ($1 ~ /__/) { accession=$1; sub(/^.*__/, "", accession); }
               family=label;
               sub(/_.*/, "", family);
               if (family == "") family="custom_reference";
               print label, accession, "custom_reference", family, family, "custom_tree_refs", "metadata autogenerated from FASTA header; use Family_description__accession naming", "OK";
             }' \
        >> "$TREE_REF_METADATA"
fi
...
if step_done 7; then
    ...
    cat "$CANDIDATES_FAA" "$TREE_REFS_FAA" > "$COMBINED"
...
log "  Tree references : $TREE_REFS_FAA"
log "  Tree metadata   : $TREE_REF_METADATA"
log "  Tree            : ${TREE_PREFIX}.treefile"
