...
SCORING DESIGN (from ChatGPT review):
  - Tree score capped at +2 (was +3) to prevent phylogeny overriding biochemistry
...
def parse_args():
    ...
    p.add_argument("--hmss2_dir",     default=None,
                   help="Directory with HMSS2 results (03_hmmer/hmss2/). "
                        "If supplied, hmss2_operon.tsv columns are appended "
                        "to the summary table as annotation (not scored).")
...
def select_best_nrfd(nrfd_ids_str, topology, all_nrfd_info):
    ...
    def candidate_score(nid):
        ...
        score = 0
        if "PF14589" in profs:
            score += 10    # high-specificity profile
...
def classify_topology(tm_class):
    ...
    FIXED: use explicit string comparison, not startswith or 'in',
    to avoid substring false-positives (review item #1).
    """
...
def score_classification(row):
    """
    Evidence-based scoring with the following design decisions (post-review):

    1. Topology bug fixed: tm_canonical from classify_topology() is used
    2. Tree weight capped at +2 (was +3) to prevent phylogeny overriding biochem
    3. Tree score only applied when Mo-bisPGD present (can't trust for non-Mo enzymes)
    ...
    5. PF14589 NrfD hit gets extra +1 over plain PF03916
    """
    ...
    if tm_canonical == "PsrC":
        ...
        if has_pf14589:
            base = "NrfD_PF14589(+)"        # high-specificity hit, no topology
        else:
            base = "NrfD_PF03916(topology_ND)"  # broad hit only, topology not run/unresolved
        evidence.append(base)
        score += (2 if has_pf14589 else 1)

    # --- PsrB (supporting only, never penalised) ---
    if has_psrB:
    ...
    if tree_clade and tree_clade not in ("unassigned", "inspect_tree_manually") and has_mo:
        ...
        if "PsrA" in tree_clade:
            ...
            score -= 1    # reduced from -2; let biochemical evidence dominate
...
def make_html_table(rows, out_path, hmss2_cols=None):
    ...
    if hmss2_cols:
        html.append("<p style='color:#555;font-size:10px'>"
                    ...
                    "not used in scoring.</p>")
    html += [
        "<table><tr>",
        "<th>Protein ID</th><th>Bin</th><th>Mo-bisPGD</th><th>TAT</th>",
    ...
    primary_cols = ["prot_id", "bin_name", "has_PF00384", "has_TAT_signal",
...
def main():
    ...
    # Load HMSS2 annotation table (optional)
    hmss2_data = {}   # psrA_id → dict of HMSS2 columns
    hmss2_cols = []   # column names to append, in order
    if args.hmss2_dir:
        ...
        if os.path.exists(operon_tsv):
            ...
            # Derive column list from header (skip the key col 'psrA_id')
    ...
    # Try the index path argument; fall back to looking for it relative to outdir
    index_path = args.protein_index
    if not index_path:
        # Common relative path from 09_summary back to 00_scan
    ...
    if not bin_map:
        ...
        print("         Pass --protein_index /path/to/00_scan/protein_to_bin_index.tsv")
    ...
    for prot_id in query_ids:
        ...
        # Best membrane subunit topology (prioritises PF14589, then classified hits)
        ...
        classification, confidence, evidence = score_classification(row)
        row["classification"] = classification
        row["confidence"]     = confidence
        row["evidence"]       = evidence

        # HMSS2 annotation columns (appended after scoring — no influence on score)
        if hmss2_cols:
    ...
    cols = ["prot_id", "bin_name", "has_PF00384", "PF00384_evalue",
    ...
    print(f"    NrfD_PF03916(topology_ND) : NrfD neighbour confirmed by broad HMM; "
          f"run DeepTMHMM to resolve to PsrC/TtrC/PhsC")
